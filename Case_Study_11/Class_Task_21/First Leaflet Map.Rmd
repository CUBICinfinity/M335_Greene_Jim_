...
```{r}
get_ogr_data <- function(data_url, layerStr, policy) {
  df <- tempfile()
  uf <- tempfile()
  download(data_url, df, mode = "wb")
  unzip(df, exdir = uf)
  ...
  ogr_data <- readOGR(shp_files, layer = layerStr, GDAL1_integer64_policy = policy)
...
leaflet(idaho) %>% 
  ...
  addCircles(lat = well_big$Latitude, lng = well_big$Longitude) %>% 
